...
public class Block_NoMallow extends Block{
   ...
   protected Block_NoMallow(Settings settings) {
      super(settings);
   ...
   public ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player, Hand hand, BlockHitResult hit) {
     ...
     if (itemStack.isEmpty() && Screen.hasShiftDown()) {
      ...
      }else if(item == Item_Init.MARSHMELLOW) {
        itemStack.decrement(1);
        Direction facing = state.get(FACING);
        world.removeBlock(pos, false);
        world.setBlockState(pos, Block_Init.MALLOWSTICK.getDefaultState().with(FACING, facing));
        return ActionResult.success(world.isClient);
     }else{
