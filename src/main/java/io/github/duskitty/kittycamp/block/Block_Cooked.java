...
public class Block_Cooked extends Block{
   ...
   public ActionResult onUse(BlockState state, World world, BlockPos pos, PlayerEntity player, Hand hand, BlockHitResult hit) {
      ...
      if (itemStack.isEmpty() && Screen.hasShiftDown()) {
         dropStack(world, pos, new ItemStack(Item_Init.STICK_COOKED, 1));
         world.removeBlock(pos, false);
         return ActionResult.success(world.isClient);
      } else if (itemStack.isEmpty()){
         dropStack(world, pos, new ItemStack(Item_Init.MARSHMELLOW_COOKED, 1));
